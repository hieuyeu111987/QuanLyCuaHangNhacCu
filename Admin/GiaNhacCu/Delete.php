...
if($id && $router->getPOST("submit")){
    ...
    if($gianhaccu->buildQueryParams(["where"=>"IDGiaNhacCu= ".$id])->delete()){
...
<html>
    ...
    <body>
        ...
        <div class="phangiua1">
            <p><b>Ban muon xoa gia nhac cu co ID : <?= $cateDetail["IDGiaNhacCu"]?></b></p>
        </div>
        <div class="phangiua2">
            <form action="<?php echo $router->createUrl('GiaNhacCu/Delete',["IDGiaNhacCu"=>$id]) ?>" method="POST">
                <input class="btn" type="submit" name="submit" value="Dong y">
                <input class="btn" onclick="window.location.href = '<?= $router->createUrl("GiaNhacCu/Index") ?>'" type="button" value="Cancel">
            </form>
        </div>
    </body>
</html>