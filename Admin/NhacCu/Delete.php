...
if($id && $router->getPOST("submit")){
    // if($loainhaccu->delete('IDLoaiNhacCu=:id',['id'=>$id])){
        // ($hoadonnhap->buildQueryParams(["where"=>"IDNhacCu= ".$id])->delete())&&
        // ($hoadonxuat->buildQueryParams(["where"=>"IDNhacCu= ".$id])->delete())&&
    if(($gianhaccu->buildQueryParams(["where"=>"IDNhacCu= ".$id])->delete())&&($nhaccu->buildQueryParams(["where"=>"IDNhacCu= ".$id])->delete())){
        $router->redirect('NhacCu/Index');
    }else{
...
<html>
    ...
    <body>
        ...
        <div class="phangiua1">
            <p><b>Ban muon xoa nhac cu : <?= $cateDetail["TenNhacCu"]?></b></p>
        </div>
        <div class="phangiua2">
            <form action="<?php echo $router->createUrl('NhacCu/Delete',["IDNhacCu"=>$id]) ?>" method="POST">
                <input class="btn" type="submit" name="submit" value="Dong y">
                <input class="btn" onclick="window.location.href = '<?= $router->createUrl("NhacCu/Index") ?>'" type="button" value="Cancel">
            </form>
        </div>
    </body>
</html>